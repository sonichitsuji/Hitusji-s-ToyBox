#> sorcery:spells/raum/weltraum/move
# ラウムの再帰移動の処理
...
execute if entity @s[scores={H.Return.Count=1..},tag=!H.Wait] as @s at @s if entity @e[tag=H.hit,sort=nearest,limit=1] at @s run function sorcery:spells/raum/weltraum/hit

# ただしエンドラ、テメーはダメだ
execute if entity @s[scores={H.Return.Count=1..},tag=!H.Wait] as @s at @s if entity @e[type=ender_dragon,distance=..6] at @s run function sorcery:spells/raum/weltraum/hit

# 再帰
execute if entity @s[scores={H.Return.Count=1..}] at @s run function sorcery:spells/raum/weltraum/move