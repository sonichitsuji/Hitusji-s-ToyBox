#> sorcery:spells/bolt/rail_cannon/cast
# 魔法発動
...
# 魔法を召喚し、位置を設定
execute anchored eyes run summon minecraft:armor_stand ~ ~ ~ {Tags:["H.Magic","H.Rail_Cannon","H.Life","H.Wait","H.Launch","H.Launch"],Small:1b,NoGravity:1b,Marker:1b,Invisible:1b}
...
# 10/(速度/20)=死亡時間 とし、飛行距離を一定にする
scoreboard players operation @n[tag=H.Launch] H.Temporaly = @s H.Speed
scoreboard players operation @n[tag=H.Launch] H.Temporaly /= $H20 H.Const
scoreboard players set @n[tag=H.Launch] H.Death.Count 10
scoreboard players operation @n[tag=H.Launch] H.Death.Count /= @n[tag=H.Launch] H.Temporaly
...
# 発射!
execute as @n[tag=H.Magic] at @s run function sorcery:spells/bolt/rail_cannon/move