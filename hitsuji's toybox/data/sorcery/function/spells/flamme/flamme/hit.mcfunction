#> sorcery:spells/flamme/flamme/hit
# ヒットしたときの処理
...
# タグ付け
execute as @e[distance=..2.5] run tag @s add H.hit

# ダメージの関数に渡す引数
data modify storage hitsuji:sorcery Damage set value "10"
data modify storage hitsuji:sorcery Damage_type set value "fireball"
data modify storage hitsuji:sorcery DeathMsg set value "焼かれた"

# 次に範囲内の奴にダメージ
execute as @e[tag=H.hit] at @s run function h_core:damage/ with storage hitsuji:sorcery
