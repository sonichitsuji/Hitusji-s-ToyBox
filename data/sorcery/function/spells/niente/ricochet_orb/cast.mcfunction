#> sorcery:spells/niente/ricochet_orb/cast
# 魔法発動
...
# 魔法を召喚し、位置を設定
execute anchored eyes run summon minecraft:armor_stand ~ ~ ~ {Tags:["H.Magic","H.Ricochet_orb","H.Life","H.Wait","H.Launch"],Small:1b,NoGravity:1b,Marker:1b,Invisible:1b}
execute at @s anchored eyes positioned ^-0.3 ^ ^0.5 run tp @n[tag=H.Launch] ~ ~ ~ ~ ~

# 魔法の詳細設定
scoreboard players set @n[tag=H.Launch] H.Speed 4
scoreboard players set @n[tag=H.Launch] H.Return.Count 4
...
# 音を鳴らす
playsound block.note_block.bell master @a ~ ~ ~ 0.5 1.4
...
execute as @n[tag=H.Magic] at @s run function sorcery:spells/niente/ricochet_orb/move with storage hitsuji:sorcery